...
mod tests {
    ...
    async fn insert_retention_rows(
        ...
        let reputation =
            insert_reputation_rows(pool, indexer_instance_id, old_ts, recent_ts).await?;
    ...
    async fn assert_retention_policy_rows(
        ...
        let ref_count: i32 = sqlx::query_scalar(
            "SELECT ref_count FROM policy_snapshot WHERE policy_snapshot_id = $1",
        )
    ...
    #[tokio::test]
    async fn job_run_rss_subscription_backfill_skips_when_maintenance_completed()
    -> anyhow::Result<()> {
        let Ok(test_db) = setup_db().await else {
            return Ok(());
        };
    ...
    #[tokio::test]
    async fn job_run_connectivity_profile_refresh_marks_low_success_as_failing()
    -> anyhow::Result<()> {
        let Ok(test_db) = setup_db().await else {
            return Ok(());
        };
