...
mod tests {
    ...
    async fn rss_poll_claim_returns_due_enabled_subscriptions_only() -> anyhow::Result<()> {
        ...
        let (subscription_disabled_instance_id, _) =
            insert_indexer_instance(pool, true, true).await?;
        ...
        let _ = insert_subscription(pool, not_due_instance_id, true, now + Duration::minutes(30))
            .await?;
        ...
        .await?;
        let _ = insert_subscription(pool, instance_disabled_id, true, now - Duration::minutes(1))
            .await?;
        let _ =
            insert_subscription(pool, rss_disabled_id, true, now - Duration::minutes(1)).await?;
    ...
    #[tokio::test]
    async fn rss_poll_apply_rate_limited_uses_retry_semantics_and_zeroes_counts()
    -> anyhow::Result<()> {
        let Ok(test_db) = setup_db().await else {
            return Ok(());
        };
    ...
    #[tokio::test]
    async fn rss_poll_apply_flaresolverr_success_promotes_challenged_to_solved()
    -> anyhow::Result<()> {
        let Ok(test_db) = setup_db().await else {
            return Ok(());
        };
