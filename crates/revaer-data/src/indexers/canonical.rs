...
mod tests {
    ...
    #[tokio::test]
    async fn canonical_prune_low_confidence_deletes_eligible_fallback_candidates()
    -> anyhow::Result<()> {
        let Ok(test_db) = setup_db().await else {
            return Ok(());
        };
    ...
    -> anyhow::Result<()> {
        ...
        let non_candidate_id =
            insert_canonical_non_candidate(pool, created_at, &source_hash).await?;
