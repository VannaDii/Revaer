...
async fn create_search_request_for_runs(pool: &PgPool, query_text: &str) -> anyhow::Result<Uuid> {
    let input = SearchRequestCreateInput {
        actor_user_public_id: None,
        query_text,
        query_type: "free_text",
...
async fn search_indexer_run_mark_failed_rate_limited_uses_retry_and_scope() -> anyhow::Result<()> {
    ...
    search_indexer_run_enqueue(pool, search_request_public_id, indexer_instance_public_id).await?;
    ...
    let state =
        fetch_search_run_state(pool, search_request_public_id, indexer_instance_public_id).await?;
...
async fn search_indexer_run_mark_failed_transient_retries_before_limit() -> anyhow::Result<()> {
    ...
    let pool = test_db.pool();
    let search_request_public_id = create_search_request_for_runs(pool, "transient retry").await?;
    force_search_request_running(pool, search_request_public_id).await?;
    let indexer_instance_public_id = create_indexer_instance_for_runs(pool).await?;
...
-> anyhow::Result<()> {
    ...
    let first = create_request_with_policy_set(pool, request_policy_set_public_id, "first").await?;
    let second =
...
async fn retention_purge_decrements_policy_snapshot_ref_count() -> anyhow::Result<()> {
    ...
    let request_policy_set_public_id = insert_request_policy_set(pool).await?;
    let created = create_request_with_policy_set(pool, request_policy_set_public_id, "old").await?;
