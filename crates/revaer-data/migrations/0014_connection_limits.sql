-- Connection and peer limit knobs for engine profiles.

ALTER TABLE public.engine_profile
    ADD COLUMN IF NOT EXISTS connections_limit INTEGER,
    ADD COLUMN IF NOT EXISTS connections_limit_per_torrent INTEGER,
    ADD COLUMN IF NOT EXISTS unchoke_slots INTEGER,
    ADD COLUMN IF NOT EXISTS half_open_limit INTEGER;

-- Drop older projections to expand signatures.
