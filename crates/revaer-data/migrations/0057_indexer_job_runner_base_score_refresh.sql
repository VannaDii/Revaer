-- Base score refresh job for recent canonicals.
...
BEGIN
    ...
    WITH candidate_canonicals AS (
        SELECT DISTINCT canonical_torrent_id
        FROM canonical_torrent_source
        WHERE last_seen_at >= cutoff_recent
    ),
    domain_map AS (
    ...
    scored AS (
        ...
        FROM canonical_torrent_source s
        JOIN candidate_canonicals c
            ON c.canonical_torrent_id = s.canonical_torrent_id
    ...
    FOR canonical_public_id IN
        ...
        WHERE EXISTS (
            SELECT 1
            FROM canonical_torrent_source s
            WHERE s.canonical_torrent_id = c.canonical_torrent_id
