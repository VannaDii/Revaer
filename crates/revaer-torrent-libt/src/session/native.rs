...
use revaer_torrent_core::{
    AddTorrent, EngineEvent, FilePriority, FileSelectionUpdate, RemoveTorrent, TorrentRateLimit,
    TorrentSource,
};
...
fn map_native_event(id: Uuid, event: NativeEvent) -> Vec<EngineEvent> {
    let NativeEvent {
        kind,
        state,
        name,
...
fn map_state(state: NativeTorrentState) -> EventState {
    match state {
...
const fn map_priority(priority: FilePriority) -> u8 {
    match priority {
        FilePriority::Skip => 0,
        FilePriority::Low => 1,
...
mod tests {
    use super::*;
    use crate::command::{EncryptionPolicy, EngineRuntimeConfig};
