...
fn build_storage_options(
    ...
    ffi::EngineStorageOptions {
        ...
        use_partfile: bool::from(config.use_partfile),
        disk_read_mode: config.disk_read_mode.map_or(0, DiskIoMode::as_i32),
        has_disk_read_mode: config.disk_read_mode.is_some(),
        disk_write_mode: config.disk_write_mode.map_or(0, DiskIoMode::as_i32),
        has_disk_write_mode: config.disk_write_mode.is_some(),
        verify_piece_hashes: bool::from(config.verify_piece_hashes),
        cache_size: config.cache_size.unwrap_or_default(),
        has_cache_size: config.cache_size.is_some(),
        cache_expiry: config.cache_expiry.unwrap_or_default(),
        has_cache_expiry: config.cache_expiry.is_some(),
        coalesce_reads: bool::from(config.coalesce_reads),
        coalesce_writes: bool::from(config.coalesce_writes),
        use_disk_cache_pool: bool::from(config.use_disk_cache_pool),
    }
}
