...
- Semantic-release command templates under `release/**` must remain lodash-template-safe. Do not embed shell `${...}` parameter-expansion syntax inside configured command strings; prefer plain shell variable references such as `"$VAR"` or move the conditional logic into a script.
