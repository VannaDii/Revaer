...
- Decision:
  ...
  - Add checked-in Sonar scope configuration so PostgreSQL migration SQL is excluded from Sonar issue, duplication, and coverage gating, generated API schema output is excluded from naming-rule noise, and repetitive Playwright contract files do not dominate duplication metrics.
...
- Consequences:
  ...
  - Trade-off: Sonar scope must be kept aligned if migration or generated-file layouts move.
- Follow-up:
